	...
	const faqData = [
		{
		  ...
		  answer:
			 "Our company is a marketplace with a huge assortment of branded shoes and clothing, including limited edition releases and collaborations. 95% of products are impossible to find in regular stores. Besides the huge assortment, we offer prices for original products 2-3 times lower than in retail stores, which is why we're so interesting to customers. The key issue is that we only operate in certain territories, so we handle delivery as intermediaries for a small commission.",
		},
		{
		  question: "How long does delivery take from our warehouse?",
		  answer:
			 "All items are stored in our warehouse in the city and delivery usually takes 1-5 days. We ship orders via courier services to make it as convenient as possible for each customer to receive their order.",
		},
		{
		...
		},
		{
		  ...
		  answer:
			 "After 2 years of operation on our telegram channel, we have over 10,000 subscribers and thousands of satisfied customers. We have a dedicated channel for customer reviews and completed orders. We work honestly and transparently, not hiding our data and orders. All packages are insured, and in case of loss, we will help you receive your item.",
		},
	 ]
	 const sizeChart = [
		{ eu: 36, cm: "22.5 см" },
	 ...
	 const steps = [
		{
		  id: "01",
		  text: "Поставь ногу на лист бумаги так, чтоб пятка касалась стены",
		  description: "Place your foot on a sheet of paper so that your heel touches the wall",
		},
		{
		  id: "02",
		  text: "Обведи ногу карандашом, не отрывая стопу от бумаги",
		  description: "Trace your foot with a pencil without lifting it from the paper",
		},
		{
		  id: "03",
		  text: "Измерь расстояние от кончика большого пальца до конца пятки",
		  description: "Measure the distance from the tip of your big toe to the end of your heel",
		},
		{
		  id: "04",
		  text: "Повтори эти же действия со второй стопой",
		  description: "Repeat these steps with your other foot",
		},
	 ]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-gray-50 p-6 rounded-xl relative">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                <button  className="bg-sky-50 hover:bg-sky-100 px-4 py-2 rounded-full cursor-pointer">
                  Shoes
                </button>
          ...
          <div className="bg-teal-500 text-white p-6 rounded-xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div>
                <button className="bg-teal-800 py-[10px] px-[20px] rounded-[10px] hover:bg-teal-900 text-white">Place Order</button>
              </div>
            </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {faqData.map((faq, index) => (
            <div key={index} className="bg-white rounded-lg overflow-hidden shadow-sm p-6">
            ...
            <TableBody>
              {sizeChart.map((size) => (
                <TableRow key={size.eu}>
                  <TableCell className="text-center">{size.eu}</TableCell>
                  <TableCell className="text-center text-[#4DD0E1]">{size.cm}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
